...
export function matches(changes: Changes, where: ConfigurationWhereClause): boolean {
  ...
  const hasAuthorMatch =
    !where.author || (author !== undefined && where.author.any.includes(author))

  const hasLabelMatch = !where.labels || labels.some(label => where.labels?.any.includes(label))
...
function matchesDiff({added, removed, lines}: Patch, {contains, adds, removes}: Diff): boolean {
  const bs = [
    contains && lines.length > 0 ? contains.some(x => lines.some(l => l.includes(x))) : true,
    adds && added.length > 0 ? adds.some(x => added.some(l => l.includes(x))) : true,
    removes && removed.length > 0 ? removes.some(x => removed.some(l => l.includes(x))) : true
  ]
