...
describe('run', () => {
  ...
  it('adds comments to PRs that match our glob patterns and patch contents', async () => {
    ...
    mockGitHubResponseChangedFiles(['script_using_truncate.sql', patchContaining('TRUNCATE')])
    mockGitHubResponsePrGet()
  ...
  it('does not comment on PRs that match our glob patterns but not patch contents', async () => {
    usingConfigYaml('patch_contains.yml')
    mockGitHubResponseChangedFiles(['script_not_using_truncate.sql', patchContaining('SELECT')])
    mockGitHubResponsePrGet()
  ...
  it('does not comment on PRs that match patch contents but not our glob patterns', async () => {
    usingConfigYaml('patch_contains.yml')
    mockGitHubResponseChangedFiles(['sql_commands.txt', patchContaining('TRUNCATE')])
    mockGitHubResponsePrGet()
...
const patchContaining = (t: string) => `@@ -132,7 +132,7 @@ module Test @@ -1000,7 +1000,7 @@ ${t}`
...
function mockGitHubResponsePrGet(details?: MockedPrDetails): void {
  const labels = details?.labels?.map(name => ({id: 'X', name, default: false})) || []
